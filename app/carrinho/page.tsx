...
export default function CartPage() {
  ...
  const shippingDiscount = subtotal >= FREE_SHIPPING_THRESHOLD ? shipping : 0

  const finalTotal = subtotal + shipping - shippingDiscount
  ...
  const handleCheckout = () => {
    ...
    if (shippingDiscount > 0) {
      message += `🎁 *Desconto Frete Grátis:* -${formatPrice(shippingDiscount)}\n`
    } else if (subtotal > 0 && subtotal < FREE_SHIPPING_THRESHOLD) {
      message += `📦 *Faltam:* ${formatPrice(FREE_SHIPPING_THRESHOLD - subtotal)} para ganhar frete grátis\n`
    }
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="glass rounded-xl p-6 sticky top-24">
              ...
              {/* Incentivo para completar frete grátis */}
              {subtotal > 0 && subtotal < FREE_SHIPPING_THRESHOLD && (
                <p className="text-sm text-orange-primary/90 mb-4">
                  Faltam {formatPrice(FREE_SHIPPING_THRESHOLD - subtotal)} para ganhar frete grátis.
                </p>
              )}
